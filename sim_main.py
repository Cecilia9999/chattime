...
class preProcessor:
    def get_train_examples(self, data_dir):
        return self._create_examples(
            os.path.join(data_dir, "simtrain.txt"))

    def get_dev_examples(self, data_dir):
        return self._create_examples(
            os.path.join(data_dir, "simdev.txt"))

    def get_test_examples(self,data_dir):
        return self._create_examples(
            os.path.join(data_dir, "simtest.txt"))
...
# tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', padding=True, truncation=True, return_tensors='pt')
...
def evaluate(args, model, val_dataloader):
    '''
    val_output_dirs = args.output_dir

    if not os.path.exists(val_output_dirs):
        os.makedirs(val_output_dirs)
...
def test():
    ...
    args_test = {"data_dir": '/content/gdrive/My Drive/nlpqa3/data/',
    ...
    test_dataset = load_and_cache_example(args_test, tokenizer, processor, 'simtest')
...
if __name__ == '__main__':
    ...
    args_train = {"data_dir": '/content/gdrive/My Drive/nlpqa3/data/',  
            "pre_train_model": 'bert-base-chinese',
            "output_dir": '/content/gdrive/My Drive/nlpqa3/output/data/',  
            "max_seq_length": 128,
            "batch_size": 32,
            "learning_rate": 6e-6,
            "epochs": 5,
    ...
    train_dataset = load_and_cache_example(args_train, tokenizer, processor, 'simtrain')
    val_dataset = load_and_cache_example(args_train, tokenizer, processor, 'simdev')
